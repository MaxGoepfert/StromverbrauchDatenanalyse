...
def cleanData(data, zeit_spalte, verbrauch_spalte):
...
if __name__ == "__main__":
    ...
    # Clean data
    data_50Hertz= cleanData(data_50Hertz, zeit_spalte, verbrauch_spalte)
    data_TransNetBW = cleanData(data_TransNetBW, zeit_spalte, verbrauch_spalte)
    ...
    # Differenz berechnen
    combined_data['Differenz'] = combined_data[f"{verbrauch_spalte}_50Hertz"] - combined_data[f"{verbrauch_spalte}_TransNetBW"]
...
gleitende_korrelation = data_50Hertz[verbrauch_spalte].rolling(window=30).corr(data_TransNetBW[verbrauch_spalte])
